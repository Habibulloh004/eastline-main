...
const Icons = () => {
  return (
    <section className="pt-5 w-[95%] lg:bg-primary rounded-xl xl:w-10/12 lg:mx-auto justify-end items-center md:justify-between mx-auto lg:flex grid  grid-cols-2 p-4 max-lg:gap-4">
      {myServiceData.map((item, idx) => {
        return (
          ...
          >
            <Image
              className="max-lg:hidden w-8 h-8"
              src={item.img}
              alt={item.title}
              width={100}
              height={100}
            />
            <Image
              className="lg:hidden w-16 h-w-16"
              src={item.imgp}
              alt={item.title}
              width={100}
              height={100}

...
const Partners = ({partner}) => {
  return (
    <section className="mt-5 space-y-3 md:bg-primary w-[95%] py-6 rounded-xl flex-col lg:w-10/12 lg:mx-auto justify-end items-start md:justify-center mx-auto gap-3">
        ...
        <CarouselContent className="md:h-full px-6 py-2 max-md:pb-6">
          {partner.map((item, index) => {
            return (
              <CarouselItem
                key={index}
                className="basis-full sm:basis-[45%] md:basis-[33.3%]"
              >
                <CustomImage
                  src={item.image}
                  alt={item.image}
